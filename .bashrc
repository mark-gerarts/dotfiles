...
function rmb {
  ...
  if [ -z "$remote_branches" ] && [ -z "$local_branches" ]; then
    ...
    if [ "$choice" == "y" ] || [ "$choice" == "Y" ]; then
      ...
      git push origin `git branch -r --merged | grep -v '/master$' | grep -v '/release$' | grep -v "/$current_branch$" | sed 's/origin\//:/g' | tr -d '\n'`
      # Remove local branches
      git branch -d `git branch --merged | grep -v 'master$' | grep -v 'release$' | grep -v "$current_branch$" | sed 's/origin\///g' | tr -d '\n'`
    else
